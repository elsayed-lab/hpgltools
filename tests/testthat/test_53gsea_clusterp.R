...
if (!identical(Sys.getenv("TRAVIS"), "true")) {
    ...
    expected <- c("GO:0000082", "GO:0000186", "GO:0001676",
                  "GO:0002118", "GO:0002225", "GO:0002252")
    actual <- head(sort(dmel_cp$enrich_go$BP_all$ID))
    test_that("Does the set of BP_all have the expected IDs?", {
        expect_equal(expected, actual)
    })

    expected <- c("GO:0005811", "GO:0034708", "GO:0035097", "GO:0044665", "GO:0071212")
    ...
    ## Gather some scores
    expected <- c(0.01108768, 0.02243582, 0.02243582, 0.02243582, 0.02243582, 0.02243582)
