...
testing <- sm(normalize_expt(pombe_expt, filter=TRUE))
...
testing <- sm(normalize_expt(pombe_expt, filter="hpgl"))
test_counts <- exprs(testing)
actual <- as.numeric(test_counts[1:10, 1])
...
testing <- sm(normalize_expt(pombe_expt, filter="pofa"))
test_counts <- exprs(testing)
...
testing <- sm(normalize_expt(pombe_expt, filter="kofa"))
test_counts <- exprs(testing)
actual <- as.numeric(test_counts[1:10, 1])
