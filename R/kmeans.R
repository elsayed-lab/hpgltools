## Time-stamp: <Fri Jul 10 16:50:34 2015 Ashton Trey Belew (abelew@gmail.com)>
...
pdf(file="images/clustering_fun.pdf")
cluster_list = list()
for (i in seq(1, k)) {
    ...
    plt = ggplot(df[df$name %in% cluster_ids,], aes(x=position, fill=stage)) + 
        geom_histogram(breaks=bins, colour='#333333') +
        facet_wrap(~ stage) +
        ggtitle(plt_title)
    print(plt)
    ...
    kable(head(averages))
    cat(sprintf("\n### Genes in cluster %d\n", i))
    kable(head(eff_subset))
    eff_subset_ids = eff_subset[,c("name","pro_rank")]
    colnames(eff_subset_ids) = c("ID","rank")
    cluster_go = try(simple_goseq(eff_subset_ids, lengths=gene_lengths, goids=go_ids))
    try(print(cluster_go$mfp_plot))
    try(print(cluster_go$bpp_plot))
    cat(sprintf("finished cluster."))
}
dev.off()
}
