...
combine_single_de_table <- function(li=NULL, ed=NULL, eb=NULL, de=NULL, ba=NULL,
                                    ...
                                    lfc_cutoff=1, p_cutoff=0.05, excludes=NULL) {
...
combine_de_data_table <- function(lilfcdt, listatsdt, include_limma,
  ...
  if (!isTRUE(include_basic) & !isTRUE(include_deseq) & !isTRUE(include_ebseq) &
      !isTRUE(include_edger) & !isTRUE(include_limma)) {
    stop("Nothing is included!")
  } else if (isTRUE(include_basic) & !isTRUE(include_deseq) & !isTRUE(include_ebseq) &
             !isTRUE(include_edger) & !isTRUE(include_limma)) {
    ...
    ## Now we get into the sets of threes
             ...
             !isTRUE(include_edger) & !isTRUE(include_limma)) {
    ## basic and ebseq
    comb <- merge(badt, ebdt, by="rownames", all.x=TRUE)
  } else if (!isTRUE(include_basic) & isTRUE(include_deseq) & isTRUE(include_ebseq) &
             !isTRUE(include_edger) & !isTRUE(include_limma)) {
    ## ebseq and deseq
    comb <- merge(delfcdt, destatsdt, by="rownames", all.x=TRUE)
    comb <- merge(comb, ebdt, by="rownames", all.x=TRUE)
  } else if (!isTRUE(include_basic) & !isTRUE(include_deseq) & isTRUE(include_ebseq) &
             isTRUE(include_edger) & !isTRUE(include_limma)) {
    ## ebseq and edger
    comb <- merge(lilfcdt, edlfcdt, by="rownames", all.x=TRUE)
    ...
    comb <- merge(comb, ebdt, by="rownames", all.x=TRUE)
    ##
    ## Now we get into the sets of fours
    ##
  } else if (!isTRUE(include_basic) & isTRUE(include_deseq) & isTRUE(include_ebseq) &
             isTRUE(include_edger) & isTRUE(include_limma)) {
