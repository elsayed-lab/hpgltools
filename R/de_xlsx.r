...
combine_de_tables <- function(apr, extra_annot=NULL,
  ...
  if (class(limma) == "try-error" | is.null(limma)) {
    include_limma <- FALSE
    add_plots <- FALSE
    message("Not adding plots, limma had an error.")
  }
  if (class(deseq) == "try-error" | is.null(deseq)) {
    include_deseq <- FALSE
    add_plots <- FALSE
    message("Not adding plots, deseq had an error.")
  }
  if (class(edger) == "try-error" | is.null(edger)) {
    include_edger <- FALSE
    add_plots <- FALSE
    message("Not adding plots, edger had an error.")
  }
  if (class(ebseq) == "try-error" | is.null(ebseq)) {
    include_ebseq <- FALSE
    compare_plots <- FALSE
  }
  if (class(basic) == "try-error" | is.null(basic)) {
    include_basic <- FALSE
    add_plots <- FALSE
  ...
  if (isTRUE(include_limma)) {
    ...
    if (found_names == 0) {
      names_length <- length(limma[["contrasts_performed"]])
      table_names <- limma[["contrasts_performed"]]
  ...
  if (isTRUE(include_deseq)) {
    ...
    if (found_names == 0) {
      names_length <- length(deseq[["contrasts_performed"]])
      table_names <- deseq[["contrasts_performed"]]
      found_names <- found_names + 1
    }
  } else {
    deseq <- NULL
  ...
  if (isTRUE(include_ebseq)) {
    ...
    if (found_names == 0) {
      names_length <- length(ebseq[["all_tables"]])
      table_names <- names(ebseq[["all_tables"]])
      found_names <- found_names + 1
    }
  } else {
    ebseq <- NULL
  ...
  if (isTRUE(include_basic)) {
    ...
    if (found_names == 0) {
      names_length <- length(names(basic[["all_tables"]]))
      table_names <- names(basic[["all_tables"]])
      found_names <- found_names + 1
    }
  } else {
    basic <- NULL
  }
  ## This copy of the table names will have its entries rewritten to take into account
  ## the preferences of the user (numerator/denominator) and thus fix the names of the
  ## comparison plot(s).
  ...
  if (isTRUE(do_excel)) {
    ...
    xl_result <- openxlsx::writeData(
                             ...
                             x=glue("PCA after surrogate estimation with: {chosen_estimate}"))
    try_result <- xlsx_plot_png(
  ...
  ## Now set up to do the more difficult work, starting by blanking out some
  ## lists to hold the data. The following will either:
  ...
  if (isTRUE(do_excel)) {
    ...
    for (x in 1:length(names(combo))) {
      tab <- names(combo)[x]
      sheetname <- tab
      ## I was getting some weird errors which magically disappeared when I did
      ## the following two lines.  This is obviously not how things are supposed
      ## to work.
...
Defaulting to fdr.")
  ...
  if (is.null(eb) | class(eb) == "try-error") {
  ...
  ebdt <- data.table::as.data.table(ebdf)
  ebdt[["rownames"]] <- rownames(ebdf)

  comb <- combine_de_data_table(
    li_lfcdt, li_statsdt, include_limma,
    de_lfcdt, de_statsdt, include_deseq,
    ed_lfcdt, ed_statsdt, include_edger,
    ebdt, include_ebseq,
    ba_statsdt, include_basic)
...
combine_de_data_table <- function(lilfcdt, listatsdt, include_limma,
  ...
  statslst <- list()
  if (isTRUE(include_deseq)) {
    datalst[["deseq"]] <- delfcdt
    statslst[["deseq"]] <- destatsdt
  }
  if (isTRUE(include_edger)) {
    datalst[["edger"]] <- edlfcdt
    statslst[["edger"]] <- edstatsdt
  }
  if (isTRUE(include_limma)) {
    datalst[["limma"]] <- lilfcdt
    statslst[["limma"]] <- listatsdt
  }
  if (isTRUE(include_ebseq)) {
    datalst[["ebseq"]] <- ebdt
  }
  ...
  if (num_data == 1) {
    if (num_stats == 1) {
      ...
      comb <- merge(datalst[[1]], statslst[[1]], by="row.names", all.x=TRUE)
    } else {
    ...
    for (i in 2:length(datalst)) {
      comb <- merge(comb, datalst[[i]], by="row.names", all.x=TRUE)
    }
    if (length(statslst) > 0) {
      for (j in 1:length(statslst)) {
        comb <- merge(comb, statslst[[j]], by="row.names", all.x=TRUE)
      }
    }
  }
