...
combine_de_tables <- function(apr, extra_annot=NULL,
  ...
  if (class(deseq)[1] == "try-error" || is.null(deseq)) {
    include_deseq <- FALSE
    add_plots <- FALSE
    message("Not adding plots, deseq had an error.")
  }
  if (class(edger)[1] == "try-error" || is.null(edger)) {
    include_edger <- FALSE
    add_plots <- FALSE
...
and is in _no_ way statistically valid, but added as a plotting conveinence.")
  ...
  if (isTRUE(do_excel)) {
    ...
    if (isTRUE(compare_plots)) {
      ...
      if (class(comp[["plot"]])[1] == "recordedplot") {
        try_result <- xlsx_plot_png(
      ...
      if (class(logfc_comparisons)[1] != "try-error") {
...
print_ups_downs <- function(upsdowns, wb=NULL, excel="excel/significant_genes.xlsx",
  ...
  for (base_name in names(ups)) {
    ...
    up_name <- glue::glue("up_{table_count}{according}_{base_name}")
    down_name <- glue::glue("down_{table_count}{according}_{base_name}")
