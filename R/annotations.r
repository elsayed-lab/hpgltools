## Time-stamp: <Tue Mar 22 16:49:52 2016 Ashton Trey Belew (abelew@gmail.com)>
...
load_parasite_annotations <- function(orgdb, gene_ids, keytype='ENSEMBL',
    ...
    gene_exons <- exonsBy(orgdb, by='gene')
    lengths <- lapply(gene_exons, function(x) { sum(width(reduce(x))) })
...
load_host_annotations <- function(orgdb, gene_ids, keytype='ENSEMBL',
    ...
    gene_info <- tbl_df(gene_info) %>%
        mutate(transcript_length=abs(TXEND - TXSTART) + 1) %>%
...
load_go_terms <- function(orgdb, gene_ids, keytype='ENSEMBL') {
    ...
    go_term_names <- suppressWarnings(
        AnnotationDbi::select(GO.db, keys=unique(go_terms$GO),
