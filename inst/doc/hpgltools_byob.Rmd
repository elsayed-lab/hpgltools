...
output:
 html_document:
  ...
  toc_float:
    ...
    smooth_scroll: false
vignette: >
  %\VignetteIndexEntry{hpgltools_introduction_byob}
...
Quite note to self, this vignette will fail utterly if I move any of the raw data on the umiacs
system, this really is not how you are supposed to do vignettes.

```{r human_macrophage_loading, cache=TRUE}
library(hpgltools)
setwd("~/scratch/rnaseq/lpanamensis")
...
hs_go_ensembl <- load_go_terms(org.Hs.eg.db, hs_annotations$geneID)
...
setwd("~/scratch/rnaseq/scerevisiae_cbf5")
sc = create_expt(file="cbf5_samples.csv", file_suffix="_forward-trimmed-v0M1.count.gz", by_sample=TRUE)
sc_genelengths = get_genelengths("reference/scerevisiae.gff.gz", try="rtracklayer::import.gff2")
sc_goids = read.delim("reference/go_trimmed.csv.gz", sep=",", header=FALSE)
...
setwd("~/scratch/rnaseq/spyogenes_mga")
...
Since writing this, clusterprofiler has changed utterly, so this won't work for the moment.

```{r simple_clusterprofiler, eval=FALSE}
setwd("~/scratch/rnaseq/scerevisiae_cbf5")
sc_clust <- simple_cp_orgdb(sc_up, orgdb="org.Sc.eg.db")
...
bioconductor tools.  It also talks a lot, perhaps I should hide its output, too.
...
```{r gprofiler}
sc_gprof <- simple_gprofiler(sc_up, species="scerevisiae")
knitr::kable(head(sc_gprof$go))
knitr::kable(head(sc_gprof$kegg))
knitr::kable(head(sc_gprof$reactome))
knitr::kable(head(sc_gprof$tf))
```
